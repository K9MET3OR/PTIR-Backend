...
class AC1_CodigoPostalPreencheLocalidade(TestCase):
    ...
    @patch('apps.taxis.views.requests.get', return_value=_mock_cp_response('LISBOA'))
    ...
    @patch('apps.taxis.views.requests.get', return_value=_mock_cp_response('PORTO'))
...
class AC2_CodigoPostalInvalido(TestCase):
    ...
    @patch('apps.taxis.views.requests.get', return_value=_mock_cp_not_found())
...
class AC3_AnoNascimentoMenoridade(TestCase):
    ...
    @patch('apps.taxis.views.requests.get', return_value=_mock_cp_response())
...
class AC4_TransacaoAtomica(TestCase):
    ...
    def test_falha_motorista_reverte_user(self, _mock):
        ...
        with patch('apps.taxis.models.Motorista.objects.create') as mock_create:
    ...
    def test_sucesso_persiste_ambos(self, _mock):
        ...
        self.assertTrue(Motorista.objects.filter(user=user).exists())
