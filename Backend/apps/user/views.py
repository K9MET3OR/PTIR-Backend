...
def create_user(request):
    ...
    if role not in ('cliente', 'motorista', 'admin'):
...
def get_user(request, id):
    ...
    # O utilizador só pode ver o seu próprio perfil, ou o admin pode ver todos
    if str(user.id) != id and user.role != 'admin':
...
def update_user(request, id):
    ...
    if 'role' in request.data:
        ...
        if request.data['role'] not in ('cliente', 'motorista', 'admin'):
            return Response({'message': 'Role inválida'}, status=400)
